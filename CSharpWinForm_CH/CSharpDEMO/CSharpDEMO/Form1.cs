    ...
    public partial class Form1 : Form
    {
       ...
       async private void button_register_Click(object sender, EventArgs e)
        ...
        async public void Func1()
        {
            while (status_flag)
            {
                ...
                try
                {
                    is_reg = readCard_no_response(0x19, 1);
                    Console.WriteLine(is_reg);
                }
                catch (Exception err)
                {
        ...
        private string readCard_no_response(byte blk_add, byte num_blk)
        {
            byte mode = 0x0000;
            string password_A = "ff ff ff ff ff ff";
            byte[] snr = new byte[6];
       ...
       async private void button_register_teacher_Click(object sender, EventArgs e)
        {
            ...
            //if (s.Unicode2String(is_reg) == "卡不存在")
            if (is_reg == "00 00 00 00 00 00 00 00 53 61 4e 0d 5b 58 57 28")
            {   MessageBox.Show("请先放上卡再进行操作");
                is_reg = ""; //为什么要清除变量呢，因为不清除会有bug.
            }
            else if(tel.Length != 11)
      ...
      async private void button2_Click(object sender, EventArgs e)
            ...
            try { 
                ...
                else
                {
                    ...
                    if (query_pay.CountAsync().Result == 0)  //查到的数据为0个,之前没有存储过
                    {
                        ...
                        string excelPath = @"D:\老师工资流水.xlsx";
                        string filename = excelPath;// @"D:\生产产量纪录.xlsx";
                                                    // open a workbook,if not exist, create a new one  
                        Excel.Workbook workBook;
                        if (File.Exists(filename))
       ...
       async private void button_savePayroll_Click(object sender, EventArgs e)
            ...
            try
            {
                ...
                else
                {
                    ...
                    try
                    {
                        ...
                        string fipath = @"d:\P" + ym + teacher.Get<string>("teacher") + ".txt";
        ...
        async private void button_courseAdd_Click(object sender, EventArgs e)
        {
            int addNum = Convert.ToInt16(textBox_courseAddName.Text);
            int addprice = Convert.ToInt16(textBox_priceAdd.Text);
            string ObID = "";
            try
            {
                //获取信息
            ...
            else
            {
                ...
                addCourse["month"] = DateTime.Now.ToString("yyyyMM");
                addCourse["student"] = myObject.Get<String>("name");
                addCourse["teacher"] = stuTeacher;
                addCourse["price"] = textBox_priceAdd.Text;
                ...
                try
                {
                    ...
                    string excelPath = @"D:\课时充值记录.xlsx";
                    string filename = excelPath;// @"D:\生产产量纪录.xlsx";
                                                // open a workbook,if not exist, create a new one  
                    Excel.Workbook workBook;
                    if (File.Exists(filename))
                    ...
                    //write data
                    workSheet = (Excel.Worksheet)workBook.Worksheets.get_Item(1);//获得第i个sheet，准备写入  
                    workSheet.Cells[1, 1] = "充值时间";
                    workSheet.Cells[1, 2] = "月份";
                    workSheet.Cells[1, 3] = "学生姓名";
                    workSheet.Cells[1, 4] = "老师姓名";
                    workSheet.Cells[1, 5] = "课程价格";
                    workSheet.Cells[1, 6] = "充值课程数目";
                    workSheet.Cells[1, 7] = "课程";
                    workSheet.Cells[1, 8] = "课程余额";
                    ...
                    workSheet.Cells[rowCount + 1, 2] = DateTime.Now.ToString("yyyyMM");
                    workSheet.Cells[rowCount + 1, 3] = name;
                    workSheet.Cells[rowCount + 1, 4] = stuTeacher;
                    workSheet.Cells[rowCount + 1, 5] = textBox_priceAdd.Text;
                    workSheet.Cells[rowCount + 1, 6] = textBox_courseAddName.Text;
                    workSheet.Cells[rowCount + 1, 7] = courseName;
                    workSheet.Cells[rowCount + 1, 8] = (times + addNum).ToString();
                    //set visible the Excel will run in background  
                    excelApp.Visible = false;
                    //set false the alerts will not display  
                    excelApp.DisplayAlerts = false;
        ...
        async private void button_registerExerciser_Click(object sender, EventArgs e)
        {
            ...
            else
            {
                try
                {
                    ...
                    string cardNum = "";
                    try {
                        cardNum = readCard(0x00, 1).Substring(0, 8);
                    }
                    catch (Exception ee) {
                        MessageBox.Show(ee.Message);
                        this.Close();
                    ...
                    string excelPath = savepath + "练习卡注册.xlsx";
                    ...
                    workSheet.Cells[rowCount + 1, 1] = date_YMD;
                    workSheet.Cells[rowCount + 1, 2] = cardNum;
                    workSheet.Cells[rowCount + 1, 3] = name;
                    workSheet.Cells[rowCount + 1, 4] = tel;
                    workSheet.Cells[rowCount + 1, 5] = "0";
       ...
       async private void button_addMoney_Click(object sender, EventArgs e)
        {
            string ObID = "";
            try
            {
            ...
            try
            {
                ...
                if (query.CountAsync().Result == 0)  //查到的数据为0个
                {
                   ...
                   // MessageBox.Show(updateCommand);
                    ...
                    string excelPath = savepath + "练习卡充值记录.xlsx";
                    string filename = excelPath;// @"D:\生.xlsx";
                                                // open a workbook,if not exist, create a new one  
                    Excel.Workbook workBook;
                    if (File.Exists(filename))
                    ...
                    workSheet.Cells[1, 3] = "姓名";
                    workSheet.Cells[1, 4] = "电话";
                    workSheet.Cells[1, 5] = "充值金额";
                    workSheet.Cells[1, 6] = "余额";
                    ...
                    workSheet.Cells[rowCount + 1, 3] = name;
                    workSheet.Cells[rowCount + 1, 4] = tel;
                    workSheet.Cells[rowCount + 1, 5] = textBox_addMoney.Text;
                    workSheet.Cells[rowCount + 1, 6] = moneyLeancloud.ToString();
        ...
        async private void button3_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                AVQuery<AVObject> queryRecord = new AVQuery<AVObject>("exerciserRecord").WhereEqualTo("objectId", ObIDrecord);
